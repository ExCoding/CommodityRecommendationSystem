...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val userRecs = preRating.filter(_.rating > 0)
      ...
      .map {
        case (userId, recs) =>
          UserRecs(userId, recs.toList.sortWith(_._2 > _._2).take(util.DataUtil.USER_MAX_RECOMMENDATION).map(x => Recommendation(x._1, x._2)))
      }
      .toDF()

    userRecs.write
      .option("uri", mongoConfig.uri)
      .option("collection", util.DataUtil.USER_RECS)
      .mode("overwrite")
      .format("com.mongodb.spark.sql")
      .save()
