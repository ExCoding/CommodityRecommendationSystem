...
object DataLoader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 加载数据
    val productRDD = spark.sparkContext.textFile(util.DataUtil.PRODUCT_DATA_PATH)
    val productDF = productRDD.map(item => {
      // product 数据通过 ^ 分隔，切分出来
      val attr = item.split("\\^")
      // 转换成 Product
      Product(attr(0).toInt, attr(1).trim, attr(4).trim, attr(5).trim, attr(6).trim)
    }).toDF()

    val ratingRDD = spark.sparkContext.textFile(util.DataUtil.RATING_DATA_PATH)
  ...
  def storeDataInMongoDB(productDF: DataFrame, ratingDF: DataFrame)(implicit mongoConfig: MongoConfig): Unit = {
    // 新建一个 mongodb 的连接，客户端
    val mongoClient = MongoClient(MongoClientURI(mongoConfig.uri))
    // 定义要操作的 mongodb 表，可以理解为 db.Product
    val productCollection = mongoClient(mongoConfig.db)(util.DataUtil.MONGODB_PRODUCT_COLLECTION)
    val ratingCollection = mongoClient(mongoConfig.db)(util.DataUtil.MONGODB_RATING_COLLECTION)
