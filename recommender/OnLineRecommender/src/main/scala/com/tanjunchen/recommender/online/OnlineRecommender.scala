package com.tanjunchen.recommender.online
...
object OnlineRecommender {
  ...
  // 定义一个连接助手对象 建立 redis 和 mongodb 的连接
  object ConnHelper extends Serializable {
    // 懒变量定义，使用的时候才初始化
    lazy val jedis = new Jedis(util.DataUtil.REDIS_URL)
    lazy val mongoClient = MongoClient(MongoClientURI(util.DataUtil.MONGO_DB_URI))
  }
  ...
  def main(args: Array[String]): Unit = {
    val config = Map(
      ...
      "kafka.topic" -> util.DataUtil.KAFKA_TOPIC
    )
    ...
    val simProductsMatrix = spark.read
      ...
      .option("collection", util.DataUtil.PRODUCT_RECS)
    ...
    // 创建kafka配置参数
    val kafkaParam = Map(
      "bootstrap.servers" -> util.DataUtil.KAFKA_BOOTSTRAP_SERVER,
      "key.deserializer" -> classOf[StringDeserializer],
      "value.deserializer" -> classOf[StringDeserializer],
      "group.id" -> util.DataUtil.KAFKA_GROUP_ID,
      ...
      rdds =>
        rdds.foreach {
          case (userId, productId, score, timestamp) =>
            ...
            val userRecentlyRatings = getUserRecentlyRatings(util.DataUtil.MAX_USER_RATING_NUM, userId, ConnHelper.jedis)

            // 2. 从相似度矩阵中获取当前商品最相似的商品列表，作为备选列表，保存成一个数组Array[productId]
            val candidateProducts = getTopSimProducts(util.DataUtil.MAX_SIM_PRODUCTS_NUM, productId, userId, simProcutsMatrixBC.value)
  ...
  def getTopSimProducts(num: Int,
    ...
    val ratingCollection = ConnHelper.mongoClient(mongoConfig.db)(util.DataUtil.MONGODB_RATING_COLLECTION)
  ...
  def saveDataToMongoDB(userId: Int, streamRecs: Array[(Int, Double)])(implicit mongoConfig: MongoConfig): Unit = {
    val streamRecsCollection = ConnHelper.mongoClient(mongoConfig.db)(util.DataUtil.STREAM_RECS)
    // 按照 userId 查询并更新
